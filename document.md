...
Pull Request 前建议在本地进行验证，提交时确保已填写 [更新日志](changelog.md)

部分软件可能无法生效

你可以在这里获取针对特定应用的[一些经验](https://github.com/HyperNavBar/HyperNavBarRules/discussions)，并分享你的经验
...
我们建议您通过 [MT管理器](https://mt2.cn/) 来获取应用 Activity：
...
本项目维护的规则文件用于 [HyperNavBar](https://github.com/HyperNavBar/HyperNavBar) 应用，通过替换系统内的导航栏沉浸规则配置文件来实现优化。

自定义规则位于仓库内的 [rules/immerse_rules.json](/rules/immerse_rules.json)，该文件基于系统的源文件进行修改和添加自定义规则所创建。
...
3. 将 `merge.json` 放到 `rules/` 目录下，提交 PR
